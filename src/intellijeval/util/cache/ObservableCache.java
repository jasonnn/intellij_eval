 ...
 */
public
class ObservableCache<K, V> extends ForwardingCache<K, V> {

    private final Cache<K, V> delegate;
    private final Set<Listener<K, V>> listeners;

    public
    ObservableCache(Cache<K, V> delegate) {
    ...
    public
    ObservableCache(Cache<K, V> delegate, Set<Listener<K, V>> listenerList) {
        this.delegate = delegate;
...
//    }
    ...
    @Override
    public
    V get(final K key, final Callable<? extends V> valueLoader) throws ExecutionException {

        return super.get(key, new Callable<V>() {
            @Override
            public
            V call() throws Exception {
                V result = valueLoader.call();
                notifyAdd(key, result);
                return result;
            }
        });
    ...
    static
    class WasItCalled<V> implements Callable<V> {
        ...
        public
        WasItCalled(Callable<? extends V> delegate) {
